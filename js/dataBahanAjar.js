// Master Data untuk Stok, Rekap, dan Tracking
...
// --- Fitur Persistence (Penyimpanan Permanen Sementara di Browser) ---
// Mengambil dataTracking dari LocalStorage jika sudah pernah ada penambahan data baru
...
if (savedTracking) {
  try {
    ...
    if (Array.isArray(parsed)) {
      ...
      dataTracking = Object.keys(parsed).map(key => ({
        [key]: parsed[key]
      }));
    }
  } catch (e) {
    console.error("Gagal memuat data tracking:", e);
  }
}

// Mengambil dataStok dari LocalStorage jika sudah pernah ada penambahan data baru
const savedStok = localStorage.getItem('sitta_dataStok');
if (savedStok) {
  try {
    dataStok = JSON.parse(savedStok);
  } catch (e) {
